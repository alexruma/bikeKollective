...
class _returnBikeState extends State<returnBike> {
  ...
  bikeReturnButton() async {
    bike.doc(widget.bikeId).update({
      ...
      if (bikeReturnFields.review != "")
        "reviews": FieldValue.arrayUnion([bikeReturnFields.review])
    });
    users
        .doc(FirebaseAuth.instance.currentUser?.uid)
        .update({'bikeCheckedOut': "", 'checkoutTime': FieldValue.delete()});
    return showDialog(
        context: context,
        useRootNavigator: false,
        builder: (BuildContext context) {
          return AlertDialog(
            title: const Text("Bike Returned"),
                        ...
                        child: DropdownRatingFormField(
                            ...
                            onSaved: (value) {
                              BikeUpdate ratingUpdate =
                                  BikeUpdate(bikeDocId: widget.bikeId);
                              ratingUpdate.updateBikeRating(
                                  value,
                                  snapshot.data[0]['rating'],
                                  snapshot.data[0]['numberOfRatings']);
